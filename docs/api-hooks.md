...
> ⚠️ **[ORDER-02] 미해결**: 현재 코드(`orderApi.js`)의 URL이 `/orders/get`으로 잘못 설정되어 있음. 명세 기준 `POST /orders`. `docs/conflicts.md` 참조.
...
> `user_id` 필드 제거됨 — 서버가 쿠키에서 사용자 식별 (ORDER-01 반영).
...
> 응답이 plain Long 텍스트이므로 `responseHandler: 'text'` 설정, `Number(res)`로 파싱.  
...
**인자**: `orderId` (string) — Snowflake ID를 `Number()` 변환 없이 string 그대로 사용
...
> `PaymentSuccessPage`에서 orderId(string)로 구독, `status`가 확정되면 주문 상세 이동.  
> `skip: !orderId` 조건으로 orderId 유효 시에만 연결.
